...
const Add = ({ token }) => {
  ...
  const [bestSeller, setBestSeller] = useState(false); // Corrected initial state
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setImage3(false);
        setImage4(false);
        setBestSeller(false); // Reset to false after successful add
      } else {
  ...
  return (
    <form onSubmit={onSubmitHandler}>
      ...
      <div className="w-full">
        ...
        <input onChange={(e)=> setName(e.target.value)} value={name} placeholder='Type here' type="text" className="w-full max-w-[500px] px-3 py-2" required />
      </div>
      <div className="w-full">
        <p className="mb-2">Product name</p>
